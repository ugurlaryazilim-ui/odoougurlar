...
class ResConfigSettings(models.TransientModel):
    ...
    def action_flo_sync_all(self):
        """Tüm aktif mağazaları senkronize et."""
        self.ensure_one()
        FloOrder = self.env['flo.order']
        # sync_orders_from_flo() doesn't return count but I can fix or use try/except.
        try:
            FloOrder.sync_orders_from_flo()
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': 'Flo Senkronizasyon',
