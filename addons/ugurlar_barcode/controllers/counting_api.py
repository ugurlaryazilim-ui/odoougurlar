...
class CountingApiController(BarcodeApiBase):
    ...
    @http.route('/ugurlar_barcode/api/count_export_excel', type='http', auth='user')
    def count_export_excel(self, count_id=None, **kw):
        """Spesifik bir sayımın detaylarını Excel'de açılacak CSV olarak indirir."""
        if not count_id:
            return request.not_found()

        # count_id formatı: {loc_id}_{op_time}
        parts = str(count_id).split('_')
        if len(parts) < 2:
            return request.not_found()
            
        loc_id = int(parts[0]) if parts[0].isdigit() else 0
        op_time = parts[1]

        # op_time YYYY-MM-DD HH:MM formatında, arama için sınırları belirle
        date_from = f"{op_time}:00"
        date_to = f"{op_time}:59"
        ...
        operations = request.env['ugurlar.barcode.operation'].sudo().search(domain)

        if not operations:
            return request.not_found()

        # CSV İçeriği Oluşturma (UTF-8 BOM ile Excel uyumlu)
        import io
        import csv
        
        output = io.StringIO()
        writer = csv.writer(output, delimiter=';') # Excel TR için noktalı virgül daha iyi sonuç verebilir
        
        # Başlıklar
        writer.writerow(['Barkod', 'Ürün', 'Adet', 'Not'])
        
        loc_name = operations[0].location_id.complete_name if operations[0].location_id else 'Bilinmeyen_Raf'
        ...
        for op in operations:
            ...
            writer.writerow([
                op.barcode or '',
                product_name or '',
                str(op.quantity or 0),
                op.notes or ''
            ])

        # UTF-8 BOM ekleyerek stringi bytes'a çevir
        # Bu Excel'in Türkçe karakterleri düzgün okumasını sağlar
        content = b'\xef\xbb\xbf' + output.getvalue().encode('utf-8')

        filename = f"Sayim_Raporu_{loc_name.replace('/', '_')}_{op_time.replace(' ', '_').replace(':', '')}.csv"
