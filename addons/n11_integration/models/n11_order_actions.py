        ...
        for order in errors:
            try:
                ...
                if order.order_status in [3, 12]:
                    # Create odoo sale order from n11_order_sync method 
        ...
        for data in store_orders.values():
            ...
            for order in data['orders']:
                if not order.order_number:
                    continue
                try:
                    # N11 getOrders endpointini saat araligiyla verebiliriz ama orderNumber a gore filtremiz yok (Payload'da startDate, endDate istiyor).
