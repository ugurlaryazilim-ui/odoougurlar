...
class N11Store(models.Model):
    ...
    def _update_cron_interval(self):
        ...
        cron = self.env.ref('n11_integration.ir_cron_n11_sync_orders', raise_if_not_found=False)
        if not cron:
            return
        ...
        if stores:
            ...
            _logger.info("N11 cron aralığı %d dakikaya güncellendi", min_interval)
        else:
            # Aktif mağaza yoksa cron'u kapat
            cron.sudo().write({'active': False})
            _logger.info("N11 aktif mağaza yok, cron devre dışı bırakıldı")
