...
class PazaramaOrderSync(models.Model):
    ...
    def sync_orders_for_store(self, store):
        ...
        while True:
            ...
            for order_json in data_list:
                ...
                _od_str = order_json.get('orderDate')
                if _od_str:
                    try:
                        order_dt = datetime.strptime(_od_str, '%Y-%m-%d %H:%M')
    ...
    def _process_order_json(self, order_json, store):
        ...
        if order_date_str:
            try:
                # Pazarama API Türkiye saati döner — Odoo UTC saklar
        ...
        for item in items:
            ...
            # orderAmount / salePrice = KDV dahil müşteri fatura tutarı
            # Odoo'ya KDV hariç yazılır, Odoo üstüne KDV ekler
            gross_price_incl = price_info.get('value', 0.0)
            quantity = item.get('quantity', 1) or 1
            vat_rate = product.get('vatRate', 0)  # ör: 10, 20, 1

            if vat_rate > 0:
                price_excl_tax = gross_price_incl / (1 + vat_rate / 100.0)
            else:
                price_excl_tax = gross_price_incl
    ...
    def _create_odoo_sale_order(self, p_order, store, ship_addr, bill_addr, customer_email, phone):
        ...
        for line in p_order.line_ids:
            product = product_map.get(line.product_code) or Product.find_by_marketplace_barcode(line.product_code) if line.product_code else None
